...
class JustinRest
{
  ...
  public function getAreas(\WP_REST_Request $request)
  {
    try {
      ...
      $areas = $wpdb->get_results("SELECT * FROM woo_justin_np_areas", ARRAY_A);
  ...
  public function getCities(\WP_REST_Request $request)
  {
    try {
      ...
      $ref = $request['ref'];
      $cities = $wpdb->get_results("SELECT * FROM woo_justin_np_cities WHERE area_ref='" . esc_attr($ref) . "' ORDER BY description", ARRAY_A);
